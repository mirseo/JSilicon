...
|-----|-----------|-------------|
| `clk`       | Input  | 시스템 클럭 신호 입력 (12 MHz) |
| `rst_n`     | Input  | Active-low reset |
| `ena`     | Input  | 칩 활성화 신호 입력 (Active-High) |
| `ui_in[7:4]` | Input  | (Manual Mode) 연산의 'A' 변수로 사용할 값 입력 (4 bits) |
| `ui_in[3:0]` | Input  | (Manual Mode) 연산의 'B' 변수로 사용할 값 입력 (4 bits) |
| `uio_in[2:0]` | Input  | (Manual Mode) 연산 코드 설정 (000:+, 001:-, 010:*, 011:/, 100:%, 101:==, 110:>, 111:<) (3 bits) |
| `uo_out[6:0]` | Output | UART의 사용 중 여부 출력 (1: Busy) |
| `uio_out[7:1]` | Output | ALU의 하위 7비트 처리 결과 출력( alu_result [6:0] ) |
| `uio_out[0]`        | Output | (9600bps) 기반 UART TX 직렬 데이터 출력 | 
